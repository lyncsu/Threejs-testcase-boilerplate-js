...
vec4 mapTexelToLinear( vec4 a ) { return a; }
vec4 emissiveMapTexelToLinear( vec4 a ) { return a; }
...
vec3 LTC_EdgeVectorFormFactor( const in vec3 va, const in vec3 vb ) {
	float x = dot( va, vb );
	float y = abs( x );
	...
	return cross( va, vb ) * theta_sintheta;
}
vec3 LTC_Evaluate( const in vec3 N, const in vec3 V, const in vec3 P, const in mat3 mInv, const in vec3 rectCoords[ 4 ] ) {
	vec3 va = rectCoords[ 1 ] - rectCoords[ 0 ];
	vec3 vb = rectCoords[ 3 ] - rectCoords[ 0 ];
	vec3 lightNormal = cross( va, vb );
